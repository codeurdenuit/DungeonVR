...
const machinery = [
  { door: { mesh: 'door1', x: 19, y: 0.54, z: 0, action: 'translateZ', value: -1, duration: 2 }, trigger: { mesh: 'trigger1', x: 17.23, y: 0, z: 1.3 }, collider: 'collider_1' },
  { door: { mesh: 'door2', x: 43.6, y: -6.5, z: -8.8, action: 'rotateY', value: -Math.PI * 2 / 3, duration: 3 }, trigger: { mesh: 'trigger1', x: 44.6, y: -6.5, z: 7.26 }, collider: 'collider_2' },
  { door: { mesh: 'door3', x: 53.63, y: -4.85, z: -15, action: 'translateY', value: 2, duration: 2 }, trigger: { mesh: 'trigger1', x: 38.11, y: -6.351, z: -19 }, collider: 'collider_3' },
  { door: { mesh: 'door4', x: 76.886, y: -11.1, z: 24.43, action: 'rotateY', value: Math.PI * 2 / 3, duration: 3 }, trigger: { mesh: null, x: 75.991, y: -15.932, z: 19.135 }, collider: 'collider_4' }
];
...
export default class TriggerManager {
  ...
  constructor(assets, materials, world) {
    ...
    for (let i = 0; i < machinery.length; i++) {  //pour chaque mécanisme
      ...
      this.animations.push({ duration: doorConf.duration, tempo: 0, action: doorConf.action, value: doorConf.value });
    }
  }

  update(dt, mobs, player, world) {
    this.triggerProcess(player);
    this.updateAnimations(dt);
  }

  triggerProcess(player) {
