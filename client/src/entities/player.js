...
export default class Player {
  ...
  constructor(materialRigid, assets, materialInvisible) {
    ...
    this.handRight = this.weapons[this.currentWeaponIndex]; //création du mesh de la main droite
    ...
    this.light = new THREE.PointLight(0xffffff, 1, 33, 5); //création d'une lumière omni pour éclairer le jeu
  ...
  takeWeapon(index, soundManager) {
    if (index !== -1) { //si c'est une arme
      if (this.handRight) { //l'arme précèdente est retirer du jeu
      ...
      this.currentWeaponIndex = index;//on met a jour l'index d'arme correspondant
      soundManager.playTakeWeapon();//son de changement d'arme
    } else { //c'est un bouclier
  ...
  update(dt, controllerRight, controllerLeft, inputs, mobs, world, camera, soundManager) {
    ...
    if (controllerRight.userData.weaponSpeed > 8) {//si mouvement brutale, je n'utilise pas l'accélération car instable
      if (this.previousSpeed === 0) { //si debut d'accélération alors le son est joué.
        ...
        soundManager.playWind(this.currentWeaponIndex);
      }
  ...
  updateHit(dt, mobs, speed, direction, soundManager) { //processus en cas de frappe du joueur
    ...
    for (let i = 0; i < mobs.length; i++) { //pour chaque mob instancié
      ...
      if (cols.length) { //si collision
        ...
        const damage = this.damages[this.currentWeaponIndex];
